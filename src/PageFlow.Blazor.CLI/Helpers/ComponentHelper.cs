...
namespace PageFlow.Blazor.CLI.Helpers
{
    public class ComponentHelper
    {
        ...
        {
            string attributeName = ConstantHelper.AttributePageFlowRoutableComponent;
        ...
        public static string GetBaseComponentByAttribute(CommandOptions option)
        {
            ...
            foreach (var filePath in csFiles)
            {
                if (HasAttribute(filePath, ConstantHelper.AttributePageFlowBaseComponent))
                {
                    return filePath;
                }
            }

            var defaultBase = GetDefaultBaseNavigator(option.Path, ConstantHelper.BasePageFlowComponentName, option.DryRun);
            return defaultBase;
        }

        public static string GetIndexComponentByAttribute(string directory)
        {
            string attributeName = ConstantHelper.AttributePageFlowIndexComponent;
            var csFiles = Directory.EnumerateFiles(directory, "*.razor.cs", SearchOption.AllDirectories);
        ...
        public static string GetDefaultBaseNavigator(string directory, string fileName, bool skip = false)
        {
            ...
            if (!csFiles.Any())
            {
                ...
                var baseNavigatorPath = Path.Combine(directory, ConstantHelper.BasePageFlowComponentName);
        ...
        private static void EnsureImportsInheritsBaseNavigator(string directory)
        {
            ...
            const string inheritsLine = "@inherits PageFlowNavigator";
